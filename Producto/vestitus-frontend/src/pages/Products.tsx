...
export default function Products() {
  ...
  const activeFilterCount = [filters.category, filters.type, filters.available, filters.collection].filter(Boolean).length
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
            {categoryCards.map((cat, idx) => {
              const isAll = idx === 0
              const isActive = !isAll && filters.category === cat.slug
              return (
                <button
                  key={cat.slug || 'all'}
                  onClick={() => {
        ...
        <div ref={productsRef}>
          <div className="flex items-end justify-between mb-8">
            ...
            {activeFilterCount > 0 && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-xs text-[var(--muted)] hover:text-red-600 transition-colors ml-4">
          ...
          {activeFilterCount > 0 && (
            <div className="flex items-center gap-2 -mt-2 mb-10 flex-wrap">
              ...
              {filters.category && (
                <button onClick={() => setFilters({ ...filters, category: '' })}
                  className="badge bg-[var(--gold)]/20 text-[var(--gold-dark)] hover:bg-[var(--gold)]/30 transition-colors flex items-center gap-1">
                  Categoría: {filters.category} <X className="h-3 w-3" />
                </button>
              )}
              {filters.type && (
                <button onClick={() => setFilters({ ...filters, type: '' })}
                  className="badge bg-[var(--gold)]/20 text-[var(--gold-dark)] hover:bg-[var(--gold)]/30 transition-colors flex items-center gap-1">
                  Tipo: {filters.type === 'rent' ? 'Arriendo' : filters.type === 'sale' ? 'Venta' : 'Ambos'} <X className="h-3 w-3" />
                </button>
              )}
              {filters.available && (
                <button onClick={() => setFilters({ ...filters, available: '' })}
                  className="badge bg-[var(--gold)]/20 text-[var(--gold-dark)] hover:bg-[var(--gold)]/30 transition-colors flex items-center gap-1">
                  {filters.available === 'true' ? 'Disponible' : 'No disponible'} <X className="h-3 w-3" />
                </button>
              )}
            </div>
          )}
                ...
                {paginated.map((product) => (
                  <Link key={product.id} to={`/products/${product.id}`} className="group">
                    <div className="bg-[var(--card)] rounded-2xl overflow-hidden border border-[var(--border)] hover-lift h-full flex flex-col">
                      <div className="relative overflow-hidden bg-[var(--bg)] aspect-[3/4]">
                        {product.images?.[0] ? (
