...
export default function Checkout() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-8 py-16">
      ...
      {!user && !guestMode && (
        <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-6 mb-8 text-center space-y-4">
          <ShoppingBag className="h-8 w-8 text-[var(--muted)] mx-auto" />
          <p className="text-sm text-[var(--muted)]">¿Cómo deseas continuar?</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link to="/login" className="bg-[var(--text)] text-white px-6 py-2.5 rounded-full text-xs tracking-[0.1em] uppercase">
              Iniciar Sesión
            </Link>
            <button onClick={() => setGuestMode(true)}
              className="border border-[var(--border)] text-[var(--text)] px-6 py-2.5 rounded-full text-xs tracking-[0.1em] uppercase hover:bg-[var(--surface)] transition-colors">
              Comprar sin Cuenta
            </button>
          </div>
        </div>
      ...
      {guestMode && (
        <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 mb-8">
          <h2 className="font-serif text-xl text-[var(--text)] mb-4">Datos Personales</h2>
          <GuestCheckoutForm />
        </div>
      )}
